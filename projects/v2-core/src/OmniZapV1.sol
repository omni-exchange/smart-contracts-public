...
import {IOmniPair} from "./interfaces/IOmniPair.sol";
import {IOmniRouter02} from "./interfaces/IOmniRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/// @author Inspiration from the work of Zapper, Beefy and PancakeSwap
contract OmniZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped ETH (WETH)
    IWETH public WETH;

    // OmniRouter interface
    IOmniRouter02 public omniRouter;
    ...
    // Address OmniRouter
    address private omniRouterAddress;
     ...
     * @param _omniRouter address of the OmniRouter contract
    ...
    constructor(
        ...
        omniRouterAddress = _omniRouter;
        omniRouter = IOmniRouter02(_omniRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /**
     * @notice Zap ETH in a WETH pool (e.g. WETH/token)
     * @param _lpToken LP token address (e.g. OMNI/WETH)
     * @param _tokenAmountOutMin minimum token amount (e.g. OMNI) to receive in the intermediary swap (e.g. ETH --> OMNI)
     */
     ...
     * @param _lpToken LP token address (e.g. OMNI/WETH)
     * @param _tokenAmountOutMin minimum token to receive (e.g. OMNI) in the intermediary swap (e.g. WETH --> OMNI)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _tokenAmountOutMin minimum amount to receive (in ETH/WETH) in the intermediary swap (e.g. OMNI --> ETH)
     */
    function zapOutETH(
        address _lpToken,
     ...
     * @param _lpToken LP token address (e.g. OMNI/USDC)
     * @param _tokenToReceive one of the 2 tokens from the LP (e.g. OMNI or USDC)
     * @param _lpTokenAmount amount of LP tokens to zap out
     * @param _tokenAmountOutMin minimum token to receive (e.g. OMNI) in the intermediary swap (e.g. USDC --> OMNI)
     */
    ...
    {
        address token0 = IOmniPair(_lpToken).token0();
        address token1 = IOmniPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IOmniPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = omniRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = omniRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
     ...
     * @return swapAmountIn amount that is expected to get swapped in intermediary swap
     * @return swapAmountOut amount that is expected to get received in intermediary swap
     * @return sellToken0 whether the token0 is sold (false --> token1 is sold in the intermediary swap)
     */
    ...
    {
        require(
            _token0ToZap == IOmniPair(_lpToken).token0() || _token0ToZap == IOmniPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IOmniPair(_lpToken).token0() || _token1ToZap == IOmniPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IOmniPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IOmniPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IOmniPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = omniRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = omniRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IOmniPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "OmniRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "OmniRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        omniRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), omniRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(omniRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = omniRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = omniRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = omniRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = omniRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = omniRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
