...
import {Actions} from "flux-periphery/src/libraries/Actions.sol";
...
import {IERC721Permit} from "flux-periphery/src/interfaces/IERC721Permit.sol";

/// @title V3 to Flux Migrator
/// @notice A contract that migrates liquidity from Omni Exchange V3 to Flux
abstract contract V3ToFluxMigrator is RouterImmutables {
    ...
    /// @dev check that the flux cl position manager call is a safe call
    ...
    function _checkFluxClPositionManagerCall(bytes calldata inputs) internal view {
        bytes4 selector;
        assembly {
            selector := calldataload(inputs.offset)
        }
        if (selector != FLUX_CL_POSITION_MANAGER.modifyLiquidities.selector) {
            revert InvalidAction(selector);
        }
    ...
    /// @dev check that the flux bin position manager call is a safe call
    ...
    function _checkFluxBinPositionManagerCall(bytes calldata inputs) internal view {
        bytes4 selector;
        assembly {
            selector := calldataload(inputs.offset)
        }
        if (selector != FLUX_BIN_POSITION_MANAGER.modifyLiquidities.selector) {
            revert InvalidAction(selector);
        }
