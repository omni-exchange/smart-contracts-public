...
import {IOmniV3Pool} from "./interfaces/IOmniV3Pool.sol";
import {IOmniV3SwapCallback} from "./interfaces/IOmniV3SwapCallback.sol";
...
/// @title Router for Omni Exchange V3 Trades
abstract contract V3SwapRouter is RouterImmutables, Permit2Payments, IOmniV3SwapCallback {
    ...
    function omniV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
        ...
        if (
            UniversalRouterHelper.computePoolAddress(
                OMNI_EXCHANGE_V3_DEPLOYER, OMNI_EXCHANGE_V3_POOL_INIT_CODE_HASH, tokenIn, tokenOut, fee
            ) != msg.sender
    ...
    /// @notice Performs an Omni Exchange V3 exact input swap
    ...
    /// @notice Performs an Omni Exchange V3 exact output swap
    /// @param recipient The recipient of the output tokens
        ...
        returns (int256 amount0Delta, int256 amount1Delta, bool zeroForOne)
    {
        ...
        (amount0Delta, amount1Delta) = IOmniV3Pool(
            UniversalRouterHelper.computePoolAddress(
                OMNI_EXCHANGE_V3_DEPLOYER, OMNI_EXCHANGE_V3_POOL_INIT_CODE_HASH, tokenIn, tokenOut, fee
            )
        ).swap(
