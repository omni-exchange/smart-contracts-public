...
import {V2SwapRouter} from "../modules/omni-exchange/v2/V2SwapRouter.sol";
import {V3SwapRouter} from "../modules/omni-exchange/v3/V3SwapRouter.sol";
import {FluxSwapRouter} from "../modules/omni-exchange/flux/FluxSwapRouter.sol";
import {Payments} from "../modules/Payments.sol";
import {RouterImmutables} from "../base/RouterImmutables.sol";
import {V3ToFluxMigrator} from "../modules/V3ToFluxMigrator.sol";
import {BytesLib} from "../libraries/BytesLib.sol";
import {Commands} from "../libraries/Commands.sol";
import {Lock} from "./Lock.sol";
import {ERC20} from "solmate/src/tokens/ERC20.sol";
import {IAllowanceTransfer} from "permit2/src/interfaces/IAllowanceTransfer.sol";
import {ActionConstants} from "flux-periphery/src/libraries/ActionConstants.sol";
import {BaseActionsRouter} from "flux-periphery/src/base/BaseActionsRouter.sol";
import {CalldataDecoder} from "flux-periphery/src/libraries/CalldataDecoder.sol";
    ...
    function dispatch(bytes1 commandType, bytes calldata inputs) internal returns (bool success, bytes memory output) {
        ...
        if (command < Commands.EXECUTE_SUB_PLAN) {
            ...
            if (command < Commands.FLUX_SWAP) {
                ...
                // 0x10 <= command < 0x21
                if (command == Commands.FLUX_SWAP) {
                    // pass the calldata provided to FluxSwapRouter._executeActions (defined in BaseActionsRouter)
                    ...
                    (success, output) = address(V3_POSITION_MANAGER).call(inputs);
                    return (success, output);
                } else if (command == Commands.FLUX_CL_INITIALIZE_POOL) {
                ...
                } else if (command == Commands.FLUX_BIN_INITIALIZE_POOL) {
                ...
                } else if (command == Commands.FLUX_CL_POSITION_CALL) {
                    _checkFluxClPositionManagerCall(inputs);
                    (success, output) = address(FLUX_CL_POSITION_MANAGER).call{value: address(this).balance}(inputs);
                    return (success, output);
                } else if (command == Commands.FLUX_BIN_POSITION_CALL) {
                    _checkFluxBinPositionManagerCall(inputs);
                    (success, output) = address(FLUX_BIN_POSITION_MANAGER).call{value: address(this).balance}(inputs);
                    return (success, output);
                } else {
