...
import {PoolIdLibrary} from "flux-core/src/types/PoolId.sol";
import {SqrtPriceMath} from "flux-core/src/pool-cl/libraries/SqrtPriceMath.sol";
...
contract CLMigrator is ICLMigrator, BaseMigrator, ReentrancyLock {
    ...
    function migrateFromV2(
        ...
        FluxCLPoolParams calldata fluxPoolParams,
        ...
        MintParams memory mintParams = MintParams({
            ...
            tickLower: fluxPoolParams.tickLower,
            tickUpper: fluxPoolParams.tickUpper,
            amount0In: uint128(amount0In),
            amount1In: uint128(amount1In),
            liquidityMin: fluxPoolParams.liquidityMin,
            recipient: fluxPoolParams.recipient,
            hookData: fluxPoolParams.hookData
        });
        (uint256 amount0Consumed, uint256 amount1Consumed) =
            _addLiquidityToTargetPool(mintParams, fluxPoolParams.deadline);

        // refund if necessary, ETH is supported by CurrencyLib
        unchecked {
            if (amount0In > amount0Consumed) {
                fluxPoolParams.poolKey.currency0.transfer(fluxPoolParams.recipient, amount0In - amount0Consumed);
            }
            if (amount1In > amount1Consumed) {
                fluxPoolParams.poolKey.currency1.transfer(fluxPoolParams.recipient, amount1In - amount1Consumed);
            }
        }
    }
