...
import {IOmniPair} from "../interfaces/external/IOmniPair.sol";
import {IV3NonfungiblePositionManager} from "../interfaces/external/IV3NonfungiblePositionManager.sol";
import {IWETH9} from "../interfaces/external/IWETH9.sol";
import {Multicall} from "./Multicall.sol";
import {Currency, CurrencyLibrary} from "flux-core/src/types/Currency.sol";
import {Owner} from "flux-core/src/Owner.sol";
...
contract BaseMigrator is IBaseMigrator, Permit2Forwarder, Multicall, SelfPermitERC721, Owner {
    ...
    /// @param token0 token0 of flux poolKey
    /// @param token1 token1 of flux poolKey
    /// @return shouldReversePair if the order of tokens from v2 pair is different from flux pair (only when WETH is involved)
    function checkTokensOrderAndMatchFromV2(address v2Pair, Currency token0, Currency token1)
        internal
        view
        returns (bool shouldReversePair)
    {
        address token0V2 = IOmniPair(v2Pair).token0();
        address token1V2 = IOmniPair(v2Pair).token1();
    ...
    /// @param token0 token0 of flux poolKey
    /// @param token1 token1 of flux poolKey
    /// @return shouldReversePair if the order of tokens from v3 pool is different from flux pair (only when WETH is involved)
    ...
    /// @param shouldReversePair if the order of tokens from v2 pair is different from flux pair (only when WETH is involved)
    /// @return amount0Received the actual amount of token0 received (in order of flux pool)
    /// @return amount1Received the actual amount of token1 received (in order of flux pool)
        ...
        returns (uint256 amount0Received, uint256 amount1Received)
    {
        ...
        (amount0Received, amount1Received) = IOmniPair(v2PoolParams.pair).burn(address(this));
        ...
        /// @notice the order may mismatch with flux pool when WETH is invovled
        /// the following check makes sure that the output always match the order of flux pool
    ...
    /// @param shouldReversePair if the order of tokens from v3 pool is different from flux pair (only when WETH is involved)
    /// @return amount0Received the actual amount of token0 received (in order of flux pool)
    /// @return amount1Received the actual amount of token1 received (in order of flux pool)
    ...
    /// @notice Check and revert if tokens from both v2/v3 and flux pair does not match
    ///         Return true if match but v2v3Token1 is WETH which should be ETH in flux pair
    /// @param v2v3Token0 token0 from v2/v3 pair
    /// @param v2v3Token1 token1 from v2/v3 pair
    /// @param infiToken0 token0 from flux pair
    /// @param infiToken1 token1 from flux pair
    /// @return shouldReversePair if the order of tokens from v2/v3 pair is different from flux pair (only when WETH is involved)
