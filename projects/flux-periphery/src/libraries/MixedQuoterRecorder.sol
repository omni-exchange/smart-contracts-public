...
/// @dev Record all token accumulation and swap direction of the transaction for non-flux pools.
/// @dev Record flux swap history list for flux pools.
library MixedQuoterRecorder {
    ...
    /// @dev uint256 internal constant SWAP_FLUX_CL = uint256(keccak256("MIXED_QUOTER_SWAP_FLUX_CL")) - 1;
    uint256 internal constant SWAP_FLUX_CL = 0x81f6dae1da0d4d29d5f77cf3c03b183b7e97eaccc3653fabd2c19a3104a66441;

    /// @dev uint256 internal constant SWAP_FLUX_BIN = uint256(keccak256("MIXED_QUOTER_SWAP_FLUX_BIN")) - 1;
    uint256 internal constant SWAP_FLUX_BIN = 0x340a2462e61d58e4a5b97c3f637efbc2620e510e6cb91e0f95ed963da50d5121;

    /// @dev uint256 internal constant SWAP_FLUX_LIST = uint256(keccak256("MIXED_QUOTER_SWAP_FLUX_LIST")) - 1;
    uint256 internal constant SWAP_FLUX_LIST = 0x038bf060f5689d4445dbb0d3ef6932e2247355bea10cdd25417a6a0ad627db90;
    ...
    /// @dev Only support one direction for same non-flux pool in one transaction.
    ...
    /// @dev Record the swap history list of flux pool.
    /// @param poolHash The hash of the pool.
    /// @param swapListBytes The swap history list bytes.
    function setFluxPoolSwapList(bytes32 poolHash, bytes memory swapListBytes) internal {
        uint256 swapListSlot = uint256(keccak256(abi.encode(poolHash, SWAP_FLUX_LIST)));
        assembly ("memory-safe") {
    ...
    /// @dev Get the swap history list of flux pool.
    /// @param poolHash The hash of the pool.
    /// @return swapListBytes The swap history list bytes.
    function getFluxPoolSwapList(bytes32 poolHash) internal view returns (bytes memory swapListBytes) {
        uint256 swapListSlot = uint256(keccak256(abi.encode(poolHash, SWAP_FLUX_LIST)));
        assembly ("memory-safe") {
    ...
    /// @dev Get the flux cl pool hash.
    /// @param key The pool key.
    /// @return poolHash The hash of the pool.
    function getFluxCLPoolHash(PoolKey memory key) internal pure returns (bytes32) {
        return keccak256(abi.encode(key, SWAP_FLUX_CL));
    }

    /// @dev Get the flux bin pool hash.
    /// @param key The pool key.
    /// @return poolHash The hash of the pool.
    function getFluxBinPoolHash(PoolKey memory key) internal pure returns (bytes32) {
        return keccak256(abi.encode(key, SWAP_FLUX_BIN));
    }
}
