...
import {PoolKey} from "flux-core/src/types/PoolKey.sol";
import {CurrencyLibrary, Currency, equals} from "flux-core/src/types/Currency.sol";
import {SafeCast} from "@openzeppelin/contracts/utils/math/SafeCast.sol";
import {TickMath} from "flux-core/src/pool-cl/libraries/TickMath.sol";
import {IQuoter} from "./interfaces/IQuoter.sol";
import {ICLQuoter} from "./pool-cl/interfaces/ICLQuoter.sol";
import {IBinQuoter} from "./pool-bin/interfaces/IBinQuoter.sol";
import {IOmniV3Pool} from "./interfaces/external/IOmniV3Pool.sol";
import {IOmniV3SwapCallback} from "./interfaces/external/IOmniV3SwapCallback.sol";
...
/// @title Provides on chain quotes for flux, V3, V2 and MixedRoute exact input swaps
...
contract MixedQuoter is IMixedQuoter, IOmniV3SwapCallback, Multicall {
    using SafeCast for *;
    using V3PoolTicksCounter for IOmniV3Pool;
    ...
    constructor(
        ...
        if (
            _factoryV3 == ZERO_ADDRESS || _factoryV2 == ZERO_ADDRESS ||  _WETH9 == ZERO_ADDRESS 
                || address(_clQuoter) == ZERO_ADDRESS || address(_binQuoter) == ZERO_ADDRESS
        ) {
    ...
    /// @inheritdoc IOmniV3SwapCallback
    function omniV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory data)
        external
        view
        override
    {
        ...
        IOmniV3Pool pool = V3SmartRouterHelper.getPool(factoryV3, tokenIn, tokenOut, fee);
    ...
    function handleV3Revert(bytes memory reason, IOmniV3Pool pool, uint256 gasEstimate)
        private
        view
        ...
        returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)
    {
        ...
        IOmniV3Pool pool = V3SmartRouterHelper.getPool(factoryV3, params.tokenIn, params.tokenOut, params.fee);
    ...
    /// @dev if withContext is true, non-flux pools (v3, v2) only support one swap direction for same pool
    function quoteMixedExactInputWithContext(
        ...
        for (uint256 actionIndex = 0; actionIndex < numActions; actionIndex++) {
            ...
            if (action == MixedQuoterActions.V2_EXACT_INPUT_SINGLE) {
            ...
            } else if (action == MixedQuoterActions.FLUX_CL_EXACT_INPUT_SINGLE) {
                QuoteMixedFluxExactInputSingleParams memory clParams =
                    abi.decode(params[actionIndex], (QuoteMixedFluxExactInputSingleParams));
                (tokenIn, tokenOut) = convertWETHToFluxNativeCurrency(clParams.poolKey, tokenIn, tokenOut);
                bool zeroForOne = tokenIn < tokenOut;
                checkFluxPoolKeyCurrency(clParams.poolKey, zeroForOne, tokenIn, tokenOut);
                ...
                // will execute all swap history of same flux pool in one transaction if withContext is true
                if (withContext) {
                    bytes32 poolHash = MixedQuoterRecorder.getFluxCLPoolHash(clParams.poolKey);
                    bytes memory swapListBytes = MixedQuoterRecorder.getFluxPoolSwapList(poolHash);
                    ...
                    MixedQuoterRecorder.setFluxPoolSwapList(poolHash, swapListBytes);
                } else {
                    (amountIn, gasEstimateForCurAction) = clQuoter.quoteExactInputSingle(swapParams);
                }
            } else if (action == MixedQuoterActions.FLUX_BIN_EXACT_INPUT_SINGLE) {
                QuoteMixedFluxExactInputSingleParams memory binParams =
                    abi.decode(params[actionIndex], (QuoteMixedFluxExactInputSingleParams));
                (tokenIn, tokenOut) = convertWETHToFluxNativeCurrency(binParams.poolKey, tokenIn, tokenOut);
                bool zeroForOne = tokenIn < tokenOut;
                checkFluxPoolKeyCurrency(binParams.poolKey, zeroForOne, tokenIn, tokenOut);
                ...
                // will execute all swap history of same flux pool in one transaction if withContext is true
                if (withContext) {
                    bytes32 poolHash = MixedQuoterRecorder.getFluxBinPoolHash(binParams.poolKey);
                    bytes memory swapListBytes = MixedQuoterRecorder.getFluxPoolSwapList(poolHash);
                    IQuoter.QuoteExactSingleParams[] memory swapHistoryList;
                    uint256 swapHistoryListLength;
                    if (swapListBytes.length > 0) {
    ...
    function checkFluxPoolKeyCurrency(PoolKey memory poolKey, bool isZeroForOne, address tokenIn, address tokenOut)
        private
        pure
    {
        Currency currency0;
    ...
    /// @notice Convert WETH to native currency for flux pools
    /// @dev for example, quote route are v3 WETH pool[token0, WETH] and flux native pool[NATIVE,token1]
    /// paths is [token0, WETH, token1], we need to convert WETH to NATIVE when quote flux pool
    function convertWETHToFluxNativeCurrency(PoolKey memory poolKey, address tokenIn, address tokenOut)
        private
        view
    ...
    /// @dev Convert native currency to WETH for Non-Flux pools.
    /// For example, quote route are flux native pool[NATIVE, token0] and v3 WETH pool[WETH, token1].
