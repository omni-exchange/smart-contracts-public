...
contract BinMigrator is IBinMigrator, BaseMigrator, ReentrancyLock {
    ...
    function migrateFromV2(
        ...
        FluxBinPoolParams calldata fluxPoolParams,
        uint256 extraAmount0,
        uint256 extraAmount1
    ) external payable override isNotLocked whenNotPaused {
        bool shouldReversePair = checkTokensOrderAndMatchFromV2(
            v2PoolParams.pair, fluxPoolParams.poolKey.currency0, fluxPoolParams.poolKey.currency1
        );
        ...
        batchAndNormalizeTokens(
            fluxPoolParams.poolKey.currency0, fluxPoolParams.poolKey.currency1, extraAmount0, extraAmount1
        );
        ...
        IBinPositionManager.BinAddLiquidityParams memory addLiquidityParams = IBinPositionManager.BinAddLiquidityParams({
            poolKey: fluxPoolParams.poolKey,
            amount0: amount0Input.toUint128(),
            amount1: amount1Input.toUint128(),
            amount0Max: fluxPoolParams.amount0Max,
            amount1Max: fluxPoolParams.amount1Max,
            activeIdDesired: fluxPoolParams.activeIdDesired,
            idSlippage: fluxPoolParams.idSlippage,
            deltaIds: fluxPoolParams.deltaIds,
            distributionX: fluxPoolParams.distributionX,
            distributionY: fluxPoolParams.distributionY,
            to: fluxPoolParams.to,
            hookData: fluxPoolParams.hookData
        });

        (uint256 amount0Consumed, uint256 amount1Consumed) =
            _addLiquidityToTargetPool(addLiquidityParams, fluxPoolParams.deadline);

        // refund if necessary, ETH is supported by CurrencyLib
        unchecked {
            if (amount0Input > amount0Consumed) {
                fluxPoolParams.poolKey.currency0.transfer(fluxPoolParams.to, amount0Input - amount0Consumed);
            }
            if (amount1Input > amount1Consumed) {
                fluxPoolParams.poolKey.currency1.transfer(fluxPoolParams.to, amount1Input - amount1Consumed);
            }
        }
    }
    ...
    function migrateFromV3(
        ...
        bool shouldReversePair = checkTokensOrderAndMatchFromV3(
            v3PoolParams.nfp, v3PoolParams.tokenId, fluxPoolParams.poolKey.currency0, fluxPoolParams.poolKey.currency1
        );
