...
import "@omni-exchange/v3-lm-pool/contracts/interfaces/IOmniV3LmPool.sol";
...
import "./interfaces/callback/IOmniV3MintCallback.sol";
import "./interfaces/callback/IOmniV3SwapCallback.sol";
import "./interfaces/callback/IOmniV3FlashCallback.sol";

/// @title Omni Exchange V3 Concentrated Liquidity Pool
contract OmniV3Pool is IOmniV3Pool {
    ...
    /// @inheritdoc IOmniV3PoolImmutables
    ...
    // @dev Split point for protocol fee values
    uint32 internal constant PROTOCOL_FEE_SP = 65536;
    // @dev Denominator for protocol fee values
    ...
    /// @notice Interface for liquidity mining pool contract
    IOmniV3LmPool public lmPool;
    ...
    /// @dev Prevents calling a function from anyone except the factory or its owner
    modifier onlyFactoryOrFactoryOwner() {
        require(msg.sender == factory || msg.sender == IOmniV3Factory(factory).owner());
        _;
    }
    ...
    constructor() {
        int24 _tickSpacing;
        (factory, token0, token1, fee, _tickSpacing) = IOmniV3PoolDeployer(msg.sender).parameters();
    ...
    /// @inheritdoc IOmniV3PoolDerivedState
    ...
    function mint(
        ...
        IOmniV3MintCallback(msg.sender).omniV3MintCallback(amount0, amount1, data);
    ...
    function swap(
        ...
        if (zeroForOne) {
            ...
            IOmniV3SwapCallback(msg.sender).omniV3SwapCallback(amount0, amount1, data);
    ...
    function flash(
        ...
        IOmniV3FlashCallback(msg.sender).omniV3FlashCallback(fee0, fee1, data);
    ...
    /// @inheritdoc IOmniV3PoolOwnerActions
